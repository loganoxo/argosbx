...
upxray() {
  url="https://github.com/loganoxo/argosbx/releases/download/argosbx/xray-$cpu"
  out="$HOME/agsbx/xray"
...
upsingbox() {
  url="https://github.com/loganoxo/argosbx/releases/download/argosbx/sing-box-$cpu"
...
cleandel() {
  for P in /proc/[0-9]*; do if [ -L "$P/exe" ]; then
...
if [ "$1" = "del" ]; then
  ...
  for P in /proc/[0-9]*; do
    ...
    case "$TARGET" in *"/agsbx/x"*) kill "$(basename "$P")" 2>/dev/null ;; esac
  done
  kill -15 $(pgrep -f 'agsbx/x' 2>/dev/null) >/dev/null 2>&1
  ...
  for P in /proc/[0-9]*; do
    [ -L "$P/exe" ] || continue
    TARGET=$(readlink -f "$P/exe" 2>/dev/null) || continue
    case "$TARGET" in *"/agsbx/s"*) kill "$(basename "$P")" 2>/dev/null ;; esac
  done
  kill -15 $(pgrep -f 'agsbx/s' 2>/dev/null) >/dev/null 2>&1
